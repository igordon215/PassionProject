...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(
        value = "select post from Post post left join fetch post.user left join fetch post.forumPost",
        countQuery = "select count(post) from Post post"
    )
    Page<Post> findAllWithToOneRelationships(Pageable pageable);

    @Query("select post from Post post left join fetch post.user left join fetch post.forumPost")
    List<Post> findAllWithToOneRelationships();

    @Query("select post from Post post left join fetch post.user left join fetch post.forumPost where post.id =:id")
